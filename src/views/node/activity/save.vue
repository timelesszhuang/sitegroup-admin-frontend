                ...
                <div>
                    <Form ref="psave" :model="form" :label-width="90" :rules="AddRule" class="node-add-form">
                        ...
                        <Form-item label="活动描述" prop="activity_summary">
                            <Card shadow>
                                <textarea class='tinymce-textarea' id="tinymceEditerSavedatile"></textarea>
                            </Card>
                        ...
                        <Form-item label="活动规则" prop="rule">
                            <Card shadow>
                                <textarea class='tinymce-textarea' id="tinymceEditerSaverule"></textarea>
                            </Card>
                            <Spin fix v-if="spinShow">
                                <Icon type="load-c" size=18 class="demo-spin-icon-load"></Icon>
                                <div>加载组件中...</div>
                        ...
                        <Form-item label="详情" prop="content">
                            <Card shadow>
                                <textarea class='tinymce-textarea' id="tinymceEditerSaveinfo"></textarea>
                            </Card>
                            <Spin fix v-if="spinShow">
                                <Icon type="load-c" size=18 class="demo-spin-icon-load"></Icon>
                                <div>加载组件中...</div>
...
<script type="text/ecmascript-6">
    ...
    export default {
        ...
        destroyed() {
            tinymce.get('tinymceEditerSavedatile').destroy();
            tinymce.get('tinymceEditerSaverule').destroy();
            tinymce.get('tinymceEditerSaveinfo').destroy();
        },
        methods: {
            addmaterial(src) {
                if (this.img === 'content') {
                    let imgsrc = '<img src=' + src + '>';
                    tinymce.get('tinymceEditerSavedatile').insertContent(imgsrc);
                } else if (this.img === 'rule') {
                    // 字段4  自定义字段 写明作用
                    let imgsrc = '<img src=' + src + '>';
                    tinymce.get('tinymceEditerSaverule').insertContent(imgsrc);
            ...
            init: function () {
                this.$nextTick(() => {
                    ...
                    this.tinymceInit(this, height, 'tinymceEditerSavedatile');
                    this.tinymceInit(this, height, 'tinymceEditerSaverule','rule');
                    this.tinymceInit(this, height, 'tinymceEditerSaveinfo','info');
                });
            },
            imgpath() {
            ...
            save() {
                this.$refs.psave.validate((valid) => {
                    if (valid) {
                        ...
                        let data = this.form;
                        let id = data.id;
                        this.apiPut('activityabout/' + id, data).then((res) => {
