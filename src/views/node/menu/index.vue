    ...
    <div>
        <div class="top">
            ...
            <Select v-model="flag" style="width:200px;" placeholder="根据栏目类型查询" label-in-value filterable clearable>
                <Option v-for="item in flag_type" :value="item.value" :key="item.id">{{ item.label }}</Option>
            </Select>
            <Select v-model="tag_id" style="width:200px;" placeholder="根据栏目分类查询" label-in-value filterable clearable
                    @on-change="changeNavtype">
                <Option v-for="item in navtype" :value="item.id" :key="item.id">{{ item.text }}</Option>
            </Select>
        ...
        <div class="menutree">
            <Collapse accordion>
                <Panel :name="item.generate_name" v-for="item in data" :key="item.id">
                    {{item.name}}
                    <Icon v-if="item.child" title="包含子菜单" type="arrow-down-c"
                    ...
                    <Button style="position: absolute;right:50px;top: 10px" @click="edit(item.id)"
                    ...
                    <div slot="content">
                        <Collapse v-if="item.child" accordion>
                            <Panel :name="childitem.generate_name" v-for="childitem in item.child" :key="childitem.id">
                                ...
                                <span title="英文名"
                                      style="display:inline-block;width: 8%">{{childitem.generate_name}}</span>
                                ...
                                <div slot="content">
                                    <Collapse v-if="childitem.child" accordion>
                                        <Panel :name="child1item.generate_name" v-for="child1item in childitem.child"
                                               :key="child1item.id">
                                            {{child1item.name}}
                                            ...
                                            <Button style="position: absolute;right:50px;top: 10px"
                                                    @click="edit(child1item.id)"
                                                    icon="ivu-icon ivu-icon-edit" size="small" type="info"></Button>
                                            <Button style="position: absolute;right:20px;top: 10px"
                                                    @click="modify(child1item.id)"
                                                    icon="ivu-icon ivu-icon-ionic" size="small"></Button>
                                            <div slot="content">
                                            </div>
                                        </Panel>
        ...
        <productadd ref="addproduct" :navtype="navtype" :pidtype="pidtype" :ptype="ptype"
                    v-on:getdata="getData"/>
        <productsave ref="saveproduct" :navtype="navtype" :pidtype="pidtype" :form="editinfo" :ptype="ptype"
                     v-on:getdata="getData"/>
        <detailadd :form="editinfo" :navtype="navtype" :pidtype="pidtype" ref="adddetails"
                   v-on:getdata="getData"/>
        <questionadd ref="addquestion" :navtype="navtype" :pidtype="pidtype" :questiontype="questiontypelist"
                     v-on:getdata="getData"/>
        <detailssave ref="savedetails" :navtype="navtype" :pidtype="pidtype" :detail="editinfo"
                     v-on:getdata="getData"/>
        <questionsave ref="savequestion" :navtype="navtype" :pidtype="pidtype" :questiontype="questiontypelist"
                      :form="editinfo" v-on:getdata="getData"/>
        <articlesave ref="savearticle" :navtype="navtype" :pidtype="pidtype" :articletype="articletypelist"
                     :form="editinfo" v-on:getdata="getData"/>
        <articleadd ref="addarticle" :navtype="navtype" :pidtype="pidtype" :articletype="articletypelist"
                    v-on:getdata="getData"/>
        <sort ref="sort" :form="info"/>
    </div>
</template>
<script>
