    ...
    <div>
        <div>
            <Modal
                ...
                <div>
                    <Form ref="childsite" :model="form" :label-width="90" :rules="AddRule" class="node-add-form">
...
<script type="text/ecmascript-6">
    ...
    export default {
        ...
        methods: {
            ...
            add () {
                this.$refs.childsite.validate((valid) => {
                    if (valid) {
                        this.modal_loading = true;
                        let data = this.form;
                        this.apiPost('childsitelist', data).then((res) => {
                            this.handleAjaxResponse(res, (data, msg) => {
                                this.modal = false;
                                this.$emit('getchilddata');
                                this.$Message.success(msg);
                                this.modal_loading = false;
                                this.$refs.childsite.resetFields();
                            }, (data, msg) => {
                                this.modal_loading = false;
